...
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "../rxing-python"))
...
try:
    ...
    print(
        f"Error importing necessary libraries. Ensure rxing is built and Pillow/NumPy are installed: {e}"
    )
...
class TestDecode(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            if not os.path.exists(cls.qr_image_path):
                print(
                    f"Attempting to create dummy test image: {cls.qr_image_path} with content '{QR_CODE_EXAMPLE_TEXT}'"
                )
                qr_matrix = rxing.encode(
                    QR_CODE_EXAMPLE_TEXT, "QR_CODE", width=200, height=200
                )
            ...
            if result and result.text == QR_CODE_EXAMPLE_TEXT:
                ...
                print(
                    f"Successfully verified/created test QR image: {cls.qr_image_path}"
                )
            else:
                print(
                    f"WARNING: Test QR image '{cls.qr_image_path}' could not be decoded or content mismatch."
                )
                if os.path.exists(
                    cls.qr_image_path
                ) and not QR_CODE_IMAGE_FILENAME.startswith("auto_generated"):
                    print(
                        f"Please ensure '{cls.qr_image_path}' contains a valid QR code with text '{QR_CODE_EXAMPLE_TEXT}'."
                    )
                elif os.path.exists(cls.qr_image_path):
                    try:
                        os.remove(cls.qr_image_path)
                        print(
                            f"Removed invalid auto-generated QR image: {cls.qr_image_path}"
                        )
                    except OSError as e_remove:
                        print(
                            f"Error removing invalid auto-generated QR image '{cls.qr_image_path}': {e_remove}"
                        )

        except Exception as e:
            print(
                f"WARNING: Could not create or verify dummy QR image for testing ('{cls.qr_image_path}'): {e}"
            )
            print("Decode tests relying on this QR image may be skipped or fail.")
            if os.path.exists(cls.qr_image_path) and QR_CODE_IMAGE_FILENAME.startswith(
                "auto_generated"
            ):
                try:
                    os.remove(cls.qr_image_path)
    ...
    def setUp(self):
        ...
        if is_qr_dependent_test and not self.__class__.qr_image_valid:
            self.skipTest(
                f"Test QR image '{self.__class__.qr_image_path}' is not valid or available. Skipping QR decode test: {self.id()}"
            )
    ...
    def test_decode_from_numpy_array_qr(self):
        # setUp will skip if self.__class__.qr_image_path is not valid
        pil_img = Image.open(self.__class__.qr_image_path).convert(
            "L"
        )  # Convert to L for predictable numpy array
    ...
    def test_decode_no_barcode(self):
        if not os.path.exists(
            self.__class__.no_barcode_image_path
        ):  # Use class attribute
            self.skipTest(
                f"Test image {self.__class__.no_barcode_image_path} not found. Skipping test."
            )
    ...
    def test_decode_invalid_path(self):
        with self.assertRaises(
            FileNotFoundError
        ):  # Or the specific error rxing_lib raises
...
class TestEncode(unittest.TestCase):
    def setUp(self):
        self.temp_dir = os.path.join(
            os.path.dirname(__file__), "temp_test_output_encode"
        )
    ...
    def test_encode_qr_default_size(self):
        ...
        data_string = "Hello"
        matrix = rxing.encode(
            data_string, format="QR_CODE"
        )  # Uses default width=5, height=5
        self.assertIsInstance(matrix, rxing.BitMatrix)
        # Check against the observed dimension from the user's feedback for "Hello"
        self.assertEqual(
            matrix.width, 29, f"Width for QR encoding '{data_string}' was not 29."
        )
        self.assertEqual(
            matrix.height, 29, f"Height for QR encoding '{data_string}' was not 29."
        )
    ...
    def test_encode_code128_custom_size(self):
        ...
        self.assertEqual(
            matrix.height,
            100,
            "Code128 BitMatrix height should typically be 1 module row.",
        )
...
class TestBitMatrixMethods(unittest.TestCase):
    def setUp(self):
        ...
        try:
            ...
            self.assertIsNotNone(
                self.matrix, "Encoding failed in setUp for TestBitMatrixMethods"
            )
            self.assertTrue(
                self.matrix.width > 0, "Encoded matrix width is 0 in setUp."
            )
        except Exception as e:
        ...
        self.temp_dir = os.path.join(
            os.path.dirname(__file__), "temp_test_output_matrix"
        )
        os.makedirs(self.temp_dir, exist_ok=True)
    ...
    def test_to_pil_image(self):
        if self.matrix is None:
            self.skipTest("Matrix not created in setUp")
    ...
    def test_to_numpy_array(self):
        ...
        self.assertEqual(
            np_array.shape, (self.matrix.height, self.matrix.width)
        )  # Numpy shape is (rows, cols)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(test_images_dir_main):
        ...
        print(
            f"Please add test images (e.g., '{QR_CODE_IMAGE_FILENAME}') to this directory for comprehensive testing."
        )
        print(
            f"A dummy '{QR_CODE_IMAGE_FILENAME}' and 'no_barcode_example.png' will be auto-generated if missing, for basic tests."
        )
